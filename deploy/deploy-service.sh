...
export certcmd="--cert ~/.kube/strivedi.crt --key ~/.kube/strivedi.key --cacert ~/.kube/ca.crt"
...
echo "Deploying service to ${https}://${kubeip}/api/v1/proxy/namespaces/${kubenamespace}/services/${SERVICENAME}"
#echo "Monitor your service at ${https}://${kubeip}/api/v1/proxy/namespaces/kube-system/services/kibana-logging/?#/discover?_a=(columns:!(log),filters:!(),index:'logstash-*',interval:auto,query:(query_string:(analyze_wildcard:!t,query:'tag:%22kubernetes.${SERVICENAME}*%22')),sort:!('@timestamp',asc))"
...
while [  $COUNTER -lt 30 ]; do
  ...
  STATUSCODE=$(curl -k --silent --output /dev/stdnull --write-out "%{http_code}" $certcmd  ${https}://${kubeip}/api/v1/proxy/namespaces/${kubenamespace}/services/${SERVICENAME}/)
